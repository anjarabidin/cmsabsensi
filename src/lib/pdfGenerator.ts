...
export function generateSlipGaji(data: SlipGajiData): jsPDF {
  ...
  doc.text('Slip Gaji Staf', pageWidth / 2, 27, { align: 'center' });
  ...
  doc.setFontSize(10);
  doc.setFont('helvetica', 'bold');
  doc.text('INFORMASI STAF', 15, yPos);
  ...
  autoTable(doc, {
    ...
    body: [
      ...
      ['BPJS Kesehatan (Staf)', formatCurrency(data.payroll.bpjs_kesehatan_employee)],
      ['BPJS Ketenagakerjaan (Staf)', formatCurrency(data.payroll.bpjs_tk_employee)],
